...
#define SMART_ENSURE( expr) \
    if ( (expr) ) ; \
    else ::smart_assert::make_assert( #expr).fatal().print_context( __FILE__, __LINE__,__SMART_ASSERT_LOCATION__, ::smart_assert::lvl_condition_assert).SMART_ASSERT_A \
    /**/
    ...
    inline void dump_context_log_detail( const assert_context & context, std::ostream & out) {
        ...
        if ( !aVals.empty() ) {
          if (aVals.size()==1)
            ...
            {
            out << "(";
            for ( size_t i = 0; i < aVals.size()-1; i += 1 )
              out << aVals[i].second << ", ";
